...
void PositionClass::grandStandCamera(bool keyDown)
{
	if (keyDown)
	{
		SetPosition(7.5f, 2.5f, 7.5f);
		m_rotationY = -135.0f;
		m_rotationX = 15.0f;
	}
}

void PositionClass::towerCamera(bool keyDown)
{
	if (keyDown)
	{
...
void PositionClass::setFanSpinning(bool keyDown)
{
	if (keyDown)
	{
		if (fanSpinning)
...
bool PositionClass::getFanSpinning()
{
	return fanSpinning;
}

void PositionClass::setCarSpinning(bool keyDown)
{
	if (keyDown)
	{
		if (carSpinning)
...
bool PositionClass::getCarSpinning()
{
	return carSpinning;
}

void PositionClass::setIsLightOn(bool keyDown)
{
	if (keyDown)
	{
		if (!lightOn)
...
bool PositionClass::getIsLightOn()
{
	return lightOn;
}

void PositionClass::sprintForward(bool keydown)
{
	float radians;
	...
	// Update the position.
	m_positionX += sinf(radians) * (m_forwardSpeed * 2.0f);
	m_positionZ += cosf(radians) * (m_forwardSpeed * 2.0f);

	return;
}

void PositionClass::sprintBackwards(bool keydown)
{
	float radians;
...
void PositionClass::playGuitarSound(bool keyDown)
{
	if (keyDown)
	{
		return;
